...
export default function Registrationscreen({ navigation }) {
    ...
    let states = [
        ...
        { pl: 'Confirm password', setter: setpassword2, st: password2 }
    ...
    useEffect(() => {
        async function Checkingtoken() {
            try {
                ...
                if (value !== null) {
                    console.log(value)
                    navigation.replace('HomeStack', { "name": null })
                } else {
                    setLoggedIn(false)
                ...
                if (msg) {
                    addMessage(`Account was deleted `)
                } 
    ...
    useEffect(() => {
        if ((username.length >= 5 && password.length >= 8 && password2.length >= 8)) {
            Animated.sequence([
                Animated.delay(50),
    ...
    async function Registerapi() {
        ...
        if (emptyFields.length >= 1) {
            addMessage("stop boss")
            return
        }
        if (password !== password2) {
            addMessage("Please, check passwords")
            return;
        }
        setIndicatorVisible(true)
        await connector.registration(null, "Post", addMessage, { username, password }, navigation)
        ...
        <View style={styles.container} onLayout={async () => { await hideAsync() }}>
            ...
            <ScrollView>
                ...
                <View style={styles.vhead}>
                    ...
                    <Text style={[commonstyles.txt, { fontSize: 24 }]}>kindly register</Text>
                    <Text style={[commonstyles.txt, { fontSize: 24 }]}>an account</Text>
                ...
                <View style={styles.vinput}>
                    ...
                    {states.map((v, i) => {
                        if ((v.pl == "Password" && v.st.length < 8)||(v.pl == "Confirm password" && v.st.length < 8) || (v.pl == "Username" && v.st.length < 5)) {
